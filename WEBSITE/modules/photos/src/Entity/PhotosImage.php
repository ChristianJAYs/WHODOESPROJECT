...
class PhotosImage extends EditorialContentEntityBase implements PhotosImageInterface {
  ...
  public function getPager($id, $type = 'album_id') {
    ...
    if ($type == 'album_id') {
      ...
      $album_data = $db->query('SELECT data FROM {photos_album} WHERE album_id = :album_id', [':album_id' => $id])->fetchField();
    ...
    foreach ($results as $result) {
      ...
      $image_view = \Drupal::entityTypeManager()->getViewBuilder('photos_image')->view($photosImage, 'pager');
      if ($stop == 1) {
