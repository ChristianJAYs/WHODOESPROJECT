...
class PhotosImageSearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface, TrustedCallbackInterface {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityTypeManagerInterface $entity_type_manager, ModuleHandlerInterface $module_handler, Config $search_settings, LanguageManagerInterface $language_manager, RendererInterface $renderer, MessengerInterface $messenger, AccountInterface $account = NULL, Connection $database_replica = NULL, SearchIndexInterface $search_index = NULL) {
    ...
    $this->addCacheTags(['photos_image_list']);
  ...
  protected function findResults() {
    ...
    $query->join('photos_image_field_data', 'p', 'p.id = i.sid AND p.langcode = i.langcode');
    $query->join('node_field_data', 'n', 'n.nid = p.album_id AND n.langcode = i.langcode');
    ...
    $this->addPhotosImageRankings($query);
  ...
  protected function prepareResults(StatementInterface $found) {
    ...
    $nodeRender = $this->entityTypeManager->getViewBuilder('node');
    $photosImageStorage = $this->entityTypeManager->getStorage('photos_image');
    $photosImageRender = $this->entityTypeManager->getViewBuilder('photos_image');
    $keys = $this->keywords;
    ...
    foreach ($found as $item) {
      ...
      $rendered .= ' ' . $this->moduleHandler->invoke('comment', 'photos_image_update_index', [$photosImage]);

      $extra = $this->moduleHandler->invokeAll('photos_image_search_result', [$photosImage]);

      $username = [
        '#theme' => 'username',
        '#account' => $photosImage->getOwner(),
      ];

      $result = [
        'link' => $photosImage->toUrl('canonical', ['absolute' => TRUE])->toString(),
        'album_title' => $node->label(),
        'title' => $photosImage->label(),
        'node' => $node,
        'photos_image' => $photosImage,
        'extra' => $extra,
        'score' => $item->calculated_score,
        'snippet' => search_excerpt($keys, $rendered, $item->langcode),
        'langcode' => $photosImage->language()->getId(),
      ];

      $this->addCacheableDependency($photosImage);
      ...
      $this->addCacheableDependency($photosImage->getOwner());
      $result += [
        'user' => $this->renderer->renderPlain($username),
        'date' => $photosImage->getChangedTime(),
      ];
    ...
    if ($ranking = $this->getRankings()) {
      ...
      foreach ($ranking as $rank => $values) {
        if (isset($this->configuration['rankings'][$rank]) && !empty($this->configuration['rankings'][$rank])) {
          ...
          $query->addScore($values['score'], $arguments, $photos_image_rank);
        }
      }
    }
  }
  ...
  public function updateIndex() {
    ...
    $query = $this->databaseReplica->select('photos_image', 'p');
    $query->addField('p', 'id');
    $query->leftJoin('search_dataset', 'sd', 'sd.sid = p.id AND sd.type = :type', [':type' => $this->getPluginId()]);
    ...
    $photosImageIds = $query->execute()->fetchCol();
    if (!$photosImageIds) {
      return;
    }
    ...
    $words = [];
    try {
      foreach ($photosImageStorage->loadMultiple($photosImageIds) as $photosImage) {
        $words += $this->indexPhotosImage($photosImage);
      }
    }
    finally {
  ...
  protected function indexPhotosImage(PhotosImage $photosImage) {
    $words = [];
    $languages = $photosImage->getTranslationLanguages();
    $node_render = $this->entityTypeManager->getViewBuilder('photos_image');

    foreach ($languages as $language) {
      $photosImage = $photosImage->getTranslation($language->getId());
      // Render the node.
      $build = $node_render->view($photosImage, 'search_index', $language->getId());
      ...
      $extra = $this->moduleHandler->invokeAll('node_update_index', [$photosImage]);
      foreach ($extra as $t) {
        $text .= $t;
      }

      // Update index, using search index "type" equal to the plugin ID.
      $words += $this->searchIndex->index($this->getPluginId(), $photosImage->id(), $language->getId(), $text, FALSE);
    }
    return $words;
  }
  ...
  public function indexStatus() {
    $total = $this->database->query('SELECT COUNT(*) FROM {photos_image}')->fetchField();
    $remaining = $this->database->query("SELECT COUNT(DISTINCT p.id) FROM {photos_image} p LEFT JOIN {search_dataset} sd ON sd.sid = p.id AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0", [':type' => $this->getPluginId()])->fetchField();
  ...
  public function searchFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    // Add albums.
    $albums = array_map(['\Drupal\Component\Utility\Html', 'escape'], $this->getAlbumNames());
  ...
  public function buildSearchUrlQuery(FormStateInterface $form_state) {
    ...
    $filters = [];
    if ($form_state->hasValue('album_id') && is_array($form_state->getValue('album_id'))) {
      // Retrieve selected album_ids - Form API sets the value of unselected
      // checkboxes to 0.
      foreach ($form_state->getValue('album_id') as $albumId) {
        if ($albumId) {
          $advanced = TRUE;
