...
class PhotosRearrangeController extends ControllerBase {
  ...
  public function contentOverview() {
    ...
    foreach ($images as $image) {
      $output .= '<li id="photos_' . $image['photos_image']->id() . '" class="photos-sort-grid ui-state-default">';
  ...
  public function ajaxRearrange() {
    ...
    if ($nid) {
      ...
      $cache_tags = array_merge($cache_tags, ['node:' . $nid, 'photos:album:' . $nid]);
    }
      ...
      if ($access) {
        ...
        foreach ($order as $image_id) {
          ...
          if ($type == 'images') {
            ...
            $this->connection->update('photos_image_field_data')
              ->fields([
                'weight' => $weight,
              ])
              ->condition('id', $id)
