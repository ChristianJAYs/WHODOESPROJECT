...
class PhotosAlbum {
  ...
  public function getCover($cover_id = NULL) {
    ...
    if (!$cover_id) {
      ...
      $db = \Drupal::database();
      $cover_id = $db->query("SELECT cover_id FROM {photos_album} WHERE album_id = :album_id", [':album_id' => $albumId])->fetchField();
    }
  ...
  public function getImages($limit = 10) {
    ...
    $term = PhotosAlbum::orderValue($column, $sort, $limit, ['column' => 'p.weight', 'sort' => 'asc']);
  ...
  public static function orderValue($field, $sort, $limit, $default = 0) {
    ...
    if (!$field && !$sort) {
      $t['order'] = !$default ? ['column' => 'p.id', 'sort' => 'desc'] : $default;
    }
    else {
  ...
  public static function getCount($type, $id = 0) {
    ...
    switch ($type) {
      ...
      case 'node_album':
        return $db->query("SELECT count FROM {photos_album} WHERE album_id = :album_id", [':album_id' => $id])->fetchField();
    }
  }
