...
class PhotosAccessTest extends BrowserTestBase {
  ...
  protected function setUp() {
    ...
    $this->drupalGet('/admin/structure/photos');
  ...
  public function testAlbumPrivacySettings() {
    ...
    $edit = [
      'privacy[viewid]' => 1,
      'privacy[updateuser]' => $this->accountViewPhotosOnly->getAccountName() . ' (' . $this->accountViewPhotosOnly->id() . ')',
    ];
    $this->updateAlbumPrivacySettings($edit);
    $file = $this->container->get('entity_type.manager')->getStorage('file')->load($photosImage->field_image->target_id);
    ...
    // Remove collaborator.
    $edit = [
      'privacy[updateremove][' . $this->accountViewPhotosOnly->id() . ']' => TRUE,
    ];
    $this->updateAlbumPrivacySettings($edit);
