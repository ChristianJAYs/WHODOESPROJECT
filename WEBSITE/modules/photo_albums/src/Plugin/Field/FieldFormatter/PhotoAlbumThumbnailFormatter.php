...
/**
 * Plugin implementation of the 'photo_album_thumbnail' formatter.
 *
 * @FieldFormatter(
 *   id = "photo_album_thumbnail",
 *   label = @Translation("Photo Album Thumbnail"),
 *   field_types = {
 *     "entity_reference"
 *   }
 * )
 */
class PhotoAlbumThumbnailFormatter extends ImageFormatter {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $protected_status = $this->getMediaEntityProtectedStatus($items->getEntity());
    ...
    foreach ($media_items as $delta => $media) {
      $elements[$delta] = [
        ...
        '#item_attributes' => ['protected' => $protected_status, 'id' => $items->getEntity()->id()],
   ...
   *
   * @return bool
   *   TRUE to indicate that entity is protected, FALSE otherwise.
   */
  protected function getMediaEntityProtectedStatus(EntityInterface $entity) {
    $image_link_setting = $this->getSetting('image_link');
    // Check if the formatter involves a link.
    return isset($entity->field_protect_album->value) ? $entity->field_protect_album->value : FALSE;
  }
